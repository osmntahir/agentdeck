...
Revizyon: 2.1 — 12 Eylül 2026 (ikinci doğrulama turu: parser bariyeri mekanizması belirlendi, kapasite ölçüme bağlandı, eylem sunumu netleşti). **Durum: tasarım revize edildi; terminal uyumluluk/performans kapısı ve gerçek CLI ilk kullanım doğrulaması açık. Koşulsuz uygulamaya hazır veya hatasız ürün iddiası yoktur.** [Wayfinder haritası](https://github.com/osmntahir/agentdeck/issues/1).

Bu belge güncel normatif sözleşmedir; ADR'ler gerekçeyi, araştırmalar tarihli kanıtı taşır. Eski GitHub resolution yorumları tarihçedir; çelişen önceki hükümler bu revizyonla yürürlükten kalkar. İki incelemedeki 15 bulgunun karşılığı [revizyon kaydında](../reviews/2026-09-12-decision-reconciliation.md). Ürün kodu bu belgeyle değişmiş sayılmaz.
...
Araştırma adayları Claude 2.1.269, Gemini 0.59.0, Codex 0.154.0; **bu sürümleri ürünün destek listesine almak için gerçek PTY kabulü gerekir**. Özellikle Gemini auth'lı resume kanıtı yoktur. Desteği doğrulanmayan preset çalıştırılabilir; doğrulanmamış otomatik resume özelliği açık gösterilmez.
...
3. **Konuşmayı sürdür:** destekli Claude/Gemini için bilinen/dayatılmış açık id ile denenir; id sadece adaydır, transcript varlığı garantisi değildir. Codex için varsayılan açık eylem kendi etkileşimli resume seçicisini açar; “Konuşma kimliğiyle sürdür” alanı açık UUID'yi kabul eder. --last/--continue ile otomatik eşleştirme yoktur. Seçici kullanıcının seçimidir; hangi konuşmayı seçeceğini uygulama uydurmaz.
...
Aynı kanıt dosyasında bu ikili, en kötü kesim noktalarında dört ardışık snapshot devrinde kesintisiz referansla eşit ekran üretti. Tarama kapsamı tamamlanmış CSI ile sınırlı kalamaz: 8-bit C1 kontrolleri, gömülü veri taşıyan DCS gövdeleri, SOS/PM/APC ve alt parametreli CSI biçimleri de tanınmalıdır; kapsam testi [G1](agentdeck-v0-validation-gates.md) içindedir. Bekleyen prefix için uzunluk ve süre üst sınırı uygulanır; sınır aşılırsa terminal representation hatası açıkça gösterilir, doğru olmayan snapshot yayımlanmaz. Eksik diziyi atarak "başarılı replay" denmez.

WS mesajları: replay-start(snapshotId,daemonId,sessionId,runId,sequence,cols,rows,formatVersion,totalBytes), sıralı replay-chunk(snapshotId,index,text), replay-end(snapshotId,chunkCount), sonra output/resize/run-ended. İstemci reset'i yalnız replay-start'ta yapar; replay-end ve son xterm write callback'i bitmeden input açılmaz. Chunk'ların text payload'u en çok 32 KiB; JSON zarfının **wire** sınırı ayrıca 256 KiB (kaçış karakterlerinin genişlemesi hesaba katılır). Her chunk'ta reset yoktur. Replay total tavanı 8 MiB; bu üst sınır kaynak kabul testinde ölçülür ve gerçek ürün için tekrar gerekçelendirilir.
...
**Terminal cevapları:** DA/DSR gibi terminal tarafından üretilen cevapların tek sahibi olmalı. Hedef daemon headless modelidir; istemci replay/viewer terminalinin otomatik cevapları PTY'ye gönderilmez. Kullanıcı input'u ile terminal response ayrımı, query yanıtları, paste/mouse/IME doğrulaması açık kabul kapısıdır. Çift cevap veya istemci yokken query kilitlenmesiyle teslim yapılamaz. OSC clipboard erişimi uygulama tarafından otomatik verilmez; URL açma yalnız kullanıcı hareketiyle allowlist edilmiş http/https üzerinden olur.

### Kaynak ve kalıcılık

Başlangıç: 32 live PTY dahil rezervasyon, 256 Session dahil arşivler. Fazla legacy kayıt kesilmez; yeni create sınır hatası verir. Her headless terminal en çok 1000 normal scrollback satırı; cols 2–300, rows 1–120. Ekran hücreleri/renkler/Unicode maliyeti ham metin baytı değildir; önceki “256 halka ~64 MiB” hesabı bu mimariye kapasite kanıtı olamaz. Bu nedenle 32 sayısı **ölçülmemiş bir tavandır ve normatif değeri G1 ölçümüne bağlıdır**: G1 raporu bu yükte kuyruk/RSS/event-loop hedeflerini karşılamazsa V0 limiti ölçülen değere indirilir ve spec güncellenir. Ürün vaadi 4–8 eşzamanlı gerçek oturumdur; 32 yalnız üst koruma sınırıdır.
...
3. Headless terminal/snapshot/preview/encoding ve kontrol yanıtları: [doğrulama kapıları](agentdeck-v0-validation-gates.md). Bu kapı geçmeden terminal temelli grid ve reconnect teslim edilmez.
...
Kabul senaryoları [doğrulama kapıları](agentdeck-v0-validation-gates.md) içinde numaralıdır. Tamamlanmış saymak için ürün entegrasyonu ve gerçek browser test sonucu gerekir. Dar sentetik xterm deneyi mimari yönü destekler; tüm terminal protokolünün doğrulandığını, 32 CLI kapasitesini veya ürünün hatasız olduğunu kanıtlamaz.
