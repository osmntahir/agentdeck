...
Bu dilimde bilinçli olarak yer almayanlar sonraki dilimlerde kapandı: terminal temsili [ADR 0008](0008-terminal-slice-implementation.md), arşiv/diff/branch/silme [ADR 0011](0011-work-result-slice-implementation.md), `environment.json` ve poll [ADR 0012](0012-scan-focus-poll-implementation.md). V0'da hâlâ kapalı olan: AgentDeck'in ürettiği konuşma kimliği (`mode = resume` ve fresh UUID); G2 insan kabulü geçmeden açılmaz. `lastLaunch.mode` fresh/picker literal komut niyetidir, kimlik üretimi değildir.
