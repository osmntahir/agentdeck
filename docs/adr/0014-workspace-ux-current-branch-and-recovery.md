...
**Açılışta geri açma.** Daemon, açılışta önceki daemon kesildiğinde `live` olan kayıtları `orphaned` olarak kalıcılaştırır ve her arşivsiz Run'ın son checkpoint'ini tarar. CLI'ın kendi “Resume…” footer'ında açık konuşma kimliği varsa Claude, Codex, Gemini, Grok, OpenCode veya Antigravity aynı Session ve cwd üzerinde doğrudan o kimlikle yeni Run'a açılır; bu kural `exited` terminal geçmişi için de geçerlidir. Kimliği olmayan kesilmiş Claude/Codex/Gemini Run'ları yalnız kendi seçicileriyle açılabilir; genel komutlar ve kimliği olmayan bitmiş işler otomatik çalışmaz. Footer dışındaki metin veya komut örneğinden kimlik çıkarılmaz. Bir hedef spawn/env hatasında bir kez denenir ve sonraki daemon açılışında kendiliğinden tekrar denenmez; kullanıcı açık “yeniden çalıştır” eylemiyle tekrar deneyebilir. Konuşma kimliği tahmini [ADR 0003](0003-restart-derived-spawn.md) gereği yapılmaz.
