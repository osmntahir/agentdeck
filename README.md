...
- **Bildirimler** — bakmadığın bir ajan onay beklediğinde, uzun bir çalışmadan sonra çıktıyı durdurduğunda veya bittiğinde haber verir. Uygulama öndeyken köşede kısa kart, arka plandayken masaüstü bildirimi çıkar; oturumu açınca kayıt kalkar. Dikkat, çıktı durulduktan sonra çizilmiş ekrandan okunur ([ADR 0016](docs/adr/0016-notifications-and-grid-shortcuts.md)).
- **Kopya ve hızlı ekleme** — oturumu Ctrl basılı tutarak grid'e sürüklemek aynı programdan yeni bir terminal açar; grid içindeki sekme için de geçerlidir. Kenar çubuğunda orta tık oturumu grid'e ekler. Grid çubuğundaki "Terminal" menüsü program başlatır veya çalışan oturumları topluca ekler. Oturum menüsünde "Aynı programla yeni oturum" vardır.
- **Kenar çubuğu** — `Ctrl+Shift+B` veya üstteki düğmeyle simge şeridine daralır; projeler renkli monogram, oturumlar ajan simgesi ve durum noktasıyla görünür.
